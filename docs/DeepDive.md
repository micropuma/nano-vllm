...
> 强烈建议参考 [Inside VLLM 博客](https://www.aleksagordic.com/blog/vllm)，其中对于paged attention kv cache等机制，提供了清晰易懂的例子。

#### KV cache + paged attention (还没有引入prefix cache)
![](./png/paged-atten-1.png)
![](./png/paged-atten-2.png)

> 重点捋清楚如下几个概念：  
> * block table 是逻辑映射结构，通常驻留在 CPU，用于调度和管理。
> * 实际的block pool在GPU端，是真正存 KV cache 的显存区域，每个 block 是一段连续显存。 
> * slot_mapping告诉flash_attn kernel，这个 query token对应的KV数据在block pool中哪个slot。

#### Prefix Cache  

![](./png/prefix-cache1.png)
![](./png/prefix-cache2.png)
...
## 一些进阶主题  
这一部分涉及VLLM项目中引入的技术，但是NanoVLLM目前尚且不支持。感兴趣的读者可以自行扩展NanoVLLM。  
